...
function afterInsert (ctx) {
  ...
  const accrualDt = rlService.calculateOrderAccrualDt({
    ...
    dateFrom: dateService.shiftDate(orderRegistry['periodID.dateFrom']),
    dateTo: dateService.shiftDate(orderRegistry['periodID.dateTo']),
  ...
  store.run('insert', {
    ...
    execParams: {
      ...
      employeeNumberID: execParams.employeeNumberID,
      payElID: execParams.payElID,
      orderDate: dateService.shiftDate(execParams.sourceDate),
      orderNumber: execParams.sourceNumber,
      ...
      mask: 0, // algorithmService.getFillMaskByPeriod(orderRegistry.dateFrom, orderRegistry.dateTo),
      paySum: execParams.paySum,
      employeeCPHID: execParams.employeeCPHID,
...
function beforeUpdate (ctx) {
  ...
  const execParams = ctx.mParams.execParams
  const store = UB.DataStore('hr_orderRegistryDt')
  const orderRegistryDt = UB.Repository('hr_orderRegistryDt').attrs(['ID', 'orderRegistryID']).where('orderID', '=', execParams.ID).selectSingle()
  const orderRegistry = UB.Repository('hr_orderRegistry')
    .attrs(['periodID', 'periodID.dateFrom', 'periodID.dateTo', 'organizationID'])
  ...
  const accrualDt = rlService.calculateOrderAccrualDt({
    ...
    paySum: execParams.paySum === undefined ? previousValues.paySum : execParams.paySum,
    dictFundSourceID: execParams.dictFundSourceID === undefined ? previousValues.dictFundSourceID : execParams.dictFundSourceID
  })
  ...
  if (orderRegistryDt) {
    store.run('update', {
      ...
      execParams: {
        ...
        employeeNumberID: execParams.employeeNumberID || previousValues.employeeNumberID,
        payElID: execParams.payElID || previousValues.payElID,
        orderDate: dateService.shiftDate(execParams.sourceDate || previousValues.sourceDate),
        orderNumber: execParams.sourceNumber || previousValues.sourceNumber,

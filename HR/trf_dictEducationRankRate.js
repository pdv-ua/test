...
me.entity.addMethod('onUpdateItem')

me.onUpdateItem = function (ctx) {
...
function beforeDelete (ctx) {
  ...
  const dictEducationRankID = instanceData.get('dictEducationRankID')
  let res = UB.Repository(__entityName)
    .attrs(['ID'])
    .where('dictEducationRankID', '=', dictEducationRankID)
...
function validateData (execParams) {
  ...
  let res = UB.Repository(__entityName)
    .attrs(['ID'])
    .where('dateFrom', '=', dateFrom)
    .where('dictEducationRankID', '=', execParams.dictEducationRankID)
  ...
  if (res) {
    ...
    maxBaseStore.run('update', {
      ...
      execParams: {
        ID: res.ID,
        dictEducationRankID: execParams.dictEducationRankID,
        dateTo: dateTo
      }
    })
  }

...
function checkDate (ctx, instanceData) {
  ...
  const payElFundSourceStore = UB.DataStore('hr_payElFundSource')
  let dateFrom = dateService.shiftDate(execParams.dateFrom || instanceData.dateFrom)
  const payElFundSource = UB.Repository('hr_payElFundSource')
  ...
  payElFundSource.forEach(row => {
    ...
    if (dateService.shiftDate(row.dateTo) >= dateFrom) {
      payElFundSourceStore.run('update', {
        skipUpdate: true,
        execParams: {
          ID: row.ID,

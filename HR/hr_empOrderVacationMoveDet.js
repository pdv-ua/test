...
me.entity.addMethod('calcVacationMoveList')

module.exports = {
  checkContiniousVacation
}

me.calcVacationMoveList = function (ctx) {
  const execParams = ctx.mParams.execParams
  const vacationList = JSON.parse(execParams.vacationList)
  const moveList = []
  let dateStart = dateService.shiftDate(execParams.dateFromProlong)
  vacationList.forEach(item => {
    item.dateFrom = dateStart
    item.dateTo = timService.getVacDateTo(dateStart, item.dayCount, item.dictVacationKindID, execParams.orgID)
    dateStart = dateService.addDays(item.dateTo, 1)
    moveList.push(item)
  })
  ctx.mParams.moveList = JSON.stringify(moveList)
}
  ...
  if (employeeNumberID && dictVacationKindID && orgID && dateService.isValid(dateFrom) && dateService.isValid(dateTo) && orderDetID) {
    ...
    if (dictVacationKindData.isDay) {
      ...
      if (totalDaysDiff < dayCount) {
        msg = UB.i18n(`Для виду відпустки '{0}' тривалість {1} дн. (сума днів за пунктами по виду відпустки за період) перевищує залишок періоду {2} дн. (залишок рахується з урахуванням всіх періодів відпустки, в яких є залишки)`, dictVacationKindData.name, dayCount, totalDaysDiff)
      }
    }
  }
  mParams.msg = msg || ''

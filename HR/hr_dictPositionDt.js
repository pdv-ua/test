...
function beforeUpdate (ctx) {
  ...
  const execParams = ctx.mParams.execParams
  execParams.dictPositionID = ctx.dataStore.get('dictPositionID')
  validateData(execParams)
}

function beforeDelete (ctx) {
  const instanceData = ctx.dataStore
  const dateFrom = instanceData.get('dateFrom')
  const dateTo = instanceData.get('dateTo')
  const res = UB.Repository(__entityName)
    .attrs(['ID'])
    .where('dateFrom', '<', dateFrom)
    .where('ID', '!=', instanceData.get('ID'))
    .where('dictPositionID', '=', instanceData.get('dictPositionID'))

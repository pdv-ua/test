...
module.exports.run = ({ cont, periodCalc, periodSalary, params, sourceAccr }) => {
  ...
  const minSalaryRec = cont.dict.hr_dictSalaryMinSize.find(o => o.dateFrom <= params.dateFrom)
  const mtCount = (params.mtCount || 1)

  const timeKoef = (!params.flagsRec || params.flagsRec & 1 << 5)
    ? (params.planHours > params.hours ? params.planHours / params.hours : (params.planHours ? 1 : 0))
    : (params.planDays > params.days ? params.planDays / params.days : (params.planDays ? 1 : 0))

  params.minSalarySum = params.flagsFix & 1 << 13 ? params.minSalarySum : (timeKoef ? ((minSalaryRec.monthValue * mtCount) / timeKoef) : 0) || 0
  let paySum = (flagsFix & 1 << 1)
    ? params.paySum
    : accrualService.roundPayEl(params.baseSum > 0 ? Math.max(0, params.minSalarySum  - params.baseSum) : 0,
  ...
  return {
    ...
    minSalarySum: params.minSalarySum,
    paySum,
    mtCount: mtCount,
    mask: params.mask,
    days: params.days,

...
module.exports.run = ({ cont, periodCalc, periodSalary, params, sourceAccr }) => {
  ...
  const paySum = (flagsFix & 1 << 1)
    ? params.paySum
    : accrualService.roundPayEl(params.baseSum *
      (params.flagsRec & 1 << 5 ? params.hours : params.days) *
  (params.koef || 1) * (cont.payEl[params.payElID].accrualRate ? cont.payEl[params.payElID].accrualRate / 100 : 1),
    cont.payEl[params.payElID].roundUpTo)
  if (sourceAccr.accrualDt) algorithmService.correctAccrualDt(sourceAccr.accrualDt, paySum)
  return {
    ...
    calcEarnings: params.calcEarnings,

...
const { generateFileName, setHeadData, getCellSettings, updateCell, updateCellInArray, createDeclarAt, buildAttrsExt, createDeclarExt, correctRowNum } = require('../../../../AC/modules/regReport/index')
...
const bodyAttrs = ['HZ', 'HZN', 'HZU', 'HZD', 'HNM', 'HNUM', 'HNUM1', 'HNAME', 'HTIN', 'HKATOTTG',
  ...
  'HKBUH', 'HBUH', 'HTELBUH',
  'T3RXXXXG02S', 'T3RXXXXG03', 'T3RXXXXG04D', 'T3RXXXXG05D', 'T3RXXXXG06S', 'T3RXXXXG07D', 'T3RXXXXG08A',
  'T3RXXXXG08', 'T3RXXXXG09A', 'T3RXXXXG09', 'T3RXXXXG10A', 'T3RXXXXG10', 'T3RXXXXG11'
]
...
const cellFormats = [
  {
    names: [
      ...
      'T3RXXXXG02S', 'T3RXXXXG03', 'T3RXXXXG04D', 'T3RXXXXG05D', 'T3RXXXXG06S', 'T3RXXXXG07D'
    ],
    format: {
      type: 'string',
      nillable: true
  ...
  },
  {
    names: [
      ...
      'R0205G03A', 'R0205G03', 'R0205G04A', 'R0205G04', 'R0205G5', 'R0205G5A',
      'R0206G03A', 'R0206G03', 'R0206G04A', 'R0206G04', 'R0206G5', 'R0206G5A',
      'T3RXXXXG08A', 'T3RXXXXG08', 'T3RXXXXG09A', 'T3RXXXXG09', 'T3RXXXXG10A', 'T3RXXXXG10'
    ],
    format: {
      type: 'number',
      nillable: true,
...
function generateData (params = {}) {
  ...
  periods.forEach((period) => {
    let idx = [0, 3, 6, 9].includes(period.dateFrom.getMonth())
    ...
    for (let i = 0; i < tabCountTaxCode + tabCountPassport; i++) {
      ...
      periodData.DECLAR.DECLARBODY.HNM = idx
      periodData.DECLAR.DECLARBODY.HNUM = params.C_DOC_TYPE
      periodData.DECLAR.DECLARHEAD.C_DOC_CNT = periodData.DECLAR.DECLARBODY.HNUM1 = idx + i
...
function prepareStructureReport () {
  ...
  data.DECLAR['$'] = {
    ...
    'xsi:noNamespaceSchemaLocation': 'J0510409.xsd'
  }
...
function prepareMainData (data, params) {
  ...
  const organization = UB.Repository('hr_organization')
    .attrs(['EDRPOUCode', 'name', 'hkatottg.code'])
  ...
  DECLARBODY.HKATOTTG = organization['hkatottg.code']
...
function addMutualSettlements (entityName, data, orgID, period) {
  const mutualSettlements = UB.Repository(`${entityName}Dt`)
    ...
    .where('docMutualSettleID.organizationID', '=', orgID)
...
function getAllData (params, periodCalc) {
  ...
  const organizations = (params.includeSubOrg && params.isIncludeOrg4)
  ...
  organizations.forEach(orgID => {
    ...
    taxPayersList.push(...getTaxPayersList(orgID, period, employeeNumbers))
    taxLimitList.push(...getTaxLimitList(orgID, period, employeeNumbers))
    militaryTaxSumList.push(...getMilitaryTaxSum(orgID, period, employeeNumbers))
    taxPayersSum.push(...getTaxPayersSum(orgID, period, employeeNumbers))
  })
  const taxPayersContainer = taxPayersSum.reduce((res, item) => {
    ...
    item.recordFlag = 0
    const taxPayer = taxPayersList.find(o => o.taxCode === item.taxCode && o.workPlace === item.workPlace)
    if (taxPayer) {
      item.dateFrom = taxPayer.dateFrom >= periodCalc.dateFrom && taxPayer.dateFrom <= periodCalc.dateTo ? taxPayer.dateFrom : null
      item.dateTo = taxPayer.dateTo <= periodCalc.dateTo ? taxPayer.dateTo : null
    }
    const taxLimit = taxLimitList.find(limit => limit.taxCode === item.taxCode && limit.code === item.code && limit.workPlace === item.workPlace)
    if (taxLimit) {
  ...
  militaryTaxSumList.forEach(mil => {
    const taxPayerData = taxPayersContainer.filter(o => o.taxCode === mil.taxCode && o.workPlace === mil.workPlace)
    if (taxPayerData.length) {
      if (mil.dtTaxCode) {
        let taxPayer = taxPayerData.find(o => o.code === mil.dtTaxCode)
        if (taxPayer) {
          taxPayer.militaryTaxSum = accrualService.round((taxPayer.militaryTaxSum || 0) + (mil.dtTaxSum || 0))
        } else {
          taxPayer = taxPayerData.sort((a, b) => {
            return a.taxSum < b.taxSum ? 1 : (a.taxSum > b.taxSum ? -1 : 0)
          })[0]
          taxPayer.militaryTaxSum = accrualService.round((taxPayer.militaryTaxSum || 0) + (mil.dtTaxSum || 0))
        }
      } else {
        const taxPayer = taxPayerData.sort((a, b) => {
          return a.taxSum < b.taxSum ? 1 : (a.taxSum > b.taxSum ? -1 : 0)
        })[0]
        taxPayer.militaryTaxSum = accrualService.round((taxPayer.militaryTaxSum || 0) + (mil.taxSum || 0))
      }
    }
  })
  ...
  if (global[`sia_docMutualSettlements`]) {
    organizations.forEach(orgID => {
      const periods = periodService.getArrayPeriods(orgID, periodCalc.dateFrom)
      const period = periods.find(o => o.dateFrom.getTime() === periodCalc.dateFrom.getTime()) || periodCalc
      addMutualSettlements(`sia_docMutualSettlements`, taxPayersContainer, orgID, period)
    })
  }

  const taxPayersContainerTaxCode = taxPayersContainer
  const taxPayersContainerPassport = []
...
function prepareData (data, params, taxPayersList, taxLimitList, taxPayersSum, taxPayersContainer, partNumber, partIndex) {
  ...
  filterTaxPayersContainer.forEach((row, idx) => {
    ...
    updateCellInArray(data, 'T1RXXXXG06D', rownum, row['dateFrom'] ? moment(row['dateFrom']).format('DDMMYYYY') : null)
    updateCellInArray(data, 'T1RXXXXG07D', rownum, row['dateTo'] ? moment(row['dateTo']).format('DDMMYYYY') : null)
...
function addTempleteForCustomRow (params) {
  ...
  params.T3 = [
    `<tr><td class="td_btn_row no-print"><button class="btn del-row no-print" data-rownum="ROWNUM" data-source="T3">X</button></td>
      <td><span class="row_num">ROWNUM</span></td>
      <td>{{#textSpanInput}}DECLAR.DECLARBODY.T3RXXXXG02S##ROWNUM##{{{}}}{{/textSpanInput}}</td>
       <td>{{#textSpanInput}}DECLAR.DECLARBODY.T3RXXXXG03##ROWNUM##{{{}}}{{/textSpanInput}}</td>
       <td>{{#dateSpanInput}}DECLAR.DECLARBODY.T3RXXXXG04D##ROWNUM##{{{}}}{{/dateSpanInput}}</td>
       <td>{{#dateSpanInput}}DECLAR.DECLARBODY.T3RXXXXG05D##ROWNUM##{{{}}}{{/dateSpanInput}}</td>
       <td>{{#textSpanInput}}DECLAR.DECLARBODY.T3RXXXXG06S##ROWNUM##{{{}}}{{/textSpanInput}}</td>
       <td>{{#dateSpanInput}}DECLAR.DECLARBODY.T3RXXXXG07D##ROWNUM##{{{}}}{{/dateSpanInput}}</td>
       <td>{{#currencySpanInput}}DECLAR.DECLARBODY.T3RXXXXG08A##ROWNUM##{{{}}}{{/currencySpanInput}}</td>
       <td>{{#currencySpanInput}}DECLAR.DECLARBODY.T3RXXXXG08##ROWNUM##{{{}}}{{/currencySpanInput}}</td>
       <td>{{#currencySpanInput}}DECLAR.DECLARBODY.T3RXXXXG09A##ROWNUM##{{{}}}{{/currencySpanInput}}</td>
       <td>{{#currencySpanInput}}DECLAR.DECLARBODY.T3RXXXXG09##ROWNUM##{{{}}}{{/currencySpanInput}}</td>
       <td>{{#currencySpanInput}}DECLAR.DECLARBODY.T3RXXXXG10A##ROWNUM##{{{}}}{{/currencySpanInput}}</td>
       <td>{{#currencySpanInput}}DECLAR.DECLARBODY.T3RXXXXG10##ROWNUM##{{{}}}{{/currencySpanInput}}</td>
       <td>{{#intSpanInput}}DECLAR.DECLARBODY.T3RXXXXG11##ROWNUM##{{{}}}{{/intSpanInput}}</td></tr>`
  ]
  params.T3BtnAddRow = [
    `<tr><td rowspan="2" class="td_btn_row no-print"><button class="btn add-row no-print" data-rownum="ROWNUM" data-source="T3" data-table="tableT3">+</button></td></tr>`
  ]
}
function getTaxPayersList (orgID, period, employeeNumbers) {
  const sqlDialect = entityBaseService.getSQLDialect()
  const store = UB.DataStore('hr_employeeNumber')
  const SQL = `
   SELECT emp.limitedAccess "limitedAccess", emp.taxCode "taxCode", emp.workPlace "workPlace", 
     MIN(emp.dateFrom) "dateFrom", MAX(emp.dateTo) "dateTo"
       
   FROM (
   SELECT e.ID, en.limitedAccess, e.taxCode, 
   (CASE WHEN en.dateFrom >= :dateFrom: AND en.dateFrom <= :dateTo: AND en.parentEmpNumberID IS NULL THEN en.dateFrom ELSE null END) dateFrom,
   (CASE WHEN en.dateTo >= :dateFrom: AND en.dateTo <= :dateTo: AND NOT EXISTS (SELECT 1 from hr_employeeNumber cn WHERE cn.parentEmpNumberID = en.ID AND cn.mi_deleteDate >= '9999-12-31')  THEN en.dateTo ELSE null END) dateTo,
   
   (CASE WHEN (select ${sqlDialect.top} ep.workPlace from hr_employeePosition ep where ep.employeeNumberID = en.ID AND ep.dateFrom <= :dateTo: and ep.isActive = 1  
    and ep.mi_deleteDate >= '9999-12-31' order by ep.dateTo desc ${sqlDialect.limit}) = '3' THEN '3' ELSE '1' END) workPlace 
   FROM hr_employeeNumber en
   JOIN hr_employee e ON e.ID = en.employeeID
   WHERE en.orgID = :orgID: AND en.dateFrom <= :dateTo: AND en.dateTo >= :dateFrom:
   ${employeeNumbers ? `AND en.ID${entityBaseService.getInExpression('employeeNumbers')}` : ''}
   AND en.empWorkPlace IS NULL 
   AND (select ${sqlDialect.top} ep.workPlace from hr_employeePosition ep where ep.employeeNumberID = en.ID AND ep.dateFrom <= :dateTo: and ep.isActive = 1  
    and ep.mi_deleteDate >= '9999-12-31' order by ep.dateTo desc ${sqlDialect.limit}) in ('1', '3')
   
   AND en.mi_deleteDate >= '9999-12-31' ) emp
   GROUP BY emp.taxCode, emp.workPlace, emp.limitedAccess
   ORDER BY emp.limitedAccess DESC 
  `
  ...
  const resultData = store.getAsJsObject()
...
function getTaxPayersSum (orgID, period, employeeNumbers) {
  const sqlDialect = entityBaseService.getSQLDialect()
  const SQL = `
  SELECT empID "empID", empTaxCodeType "empTaxCodeType", taxCode "taxCode", code, sum(incomeSum) "incomeSum", sum(taxSum) "taxSum",
  min(dateFrom), max(dateTo), limitedAccess "limitedAccess", workPlace "workPlace" 
  from (
    select 
      emp.ID as empID, 
      emp.empTaxCodeType empTaxCodeType, 
      emp.taxCode,
      empNum.limitedAccess
      ,COALESCE(dictIncom.codeReport, dictIncom.code) code
      ,tax.incomeSum
      ,tax.taxSum taxSum
      ,empNum.dateFrom
      ,empNum.dateTo
      ,(CASE WHEN (select ${sqlDialect.top} ep.workPlace from hr_employeePosition ep where ep.employeeNumberID = empNum.ID AND ep.dateFrom <= :dateTo: and ep.isActive = 1  
    and ep.mi_deleteDate >= '9999-12-31' order by ep.dateTo desc ${sqlDialect.limit}) = '3' THEN '3' ELSE '1' END) workPlace  
    ...
    where empNum.orgID = :orgID:
      ...
      AND empNum.empWorkPlace IS NULL 
      and acc.periodCalcID =:periodID:
      ${employeeNumbers ? `and empNum.ID${entityBaseService.getInExpression('employeeNumbers')}` : ''}
   ...
   select 
    ...
    select 
      ...
      empNum.limitedAccess
      ,COALESCE(dictTax.codeReport, dictTax.code) code
      ,acc.paySum incomeSum
      ,null taxSum
      ,empNum.dateFrom
      ,empNum.dateTo
    ...
    inner join hr_dictTaxIndivid dictTax on dictTax.code = '140' and dictTax.mi_deleteDate >= '9999-12-31'
    where empNum.orgID = :orgID:
    ${employeeNumbers ? `and empNum.ID${entityBaseService.getInExpression('employeeNumbers')}` : ''}
    AND empNum.empWorkPlace IS NULL 
    and acc.periodCalcID =:periodID:
  ...
  group by empID, empTaxCodeType, taxCode, code, limitedAccess, workPlace
  order by 9 DESC,3,4,7,8
`
...
function getTaxLimitList (orgID, period, employeeNumbers) {
  const sqlDialect = entityBaseService.getSQLDialect()
  const SQL = `
select
t.empTaxCodeType "empTaxCodeType",
  t.taxCode "taxCode",
  t.limitedAccess "limitedAccess",
  t.code,
  t.limit1 "limit1",
  t.limit2 "limit2",
  t.limit3 "limit3",
...
select
  ...
  empNum.limitedAccess,
  COALESCE(dictIncom.codeReport, dictIncom.code) code,
  limit1.codeForReport as limit1,
  limit2.codeForReport as limit2,
  limit3.codeForReport as limit3,
  (CASE WHEN (select ${sqlDialect.top} ep.workPlace from hr_employeePosition ep where ep.employeeNumberID = empNum.ID AND ep.dateFrom <= :dateTo: and ep.isActive = 1  
    and ep.mi_deleteDate >= '9999-12-31' order by ep.dateTo desc ${sqlDialect.limit}) = '3' THEN '3' ELSE '1' END) workPlace   
from hr_taxIndividAcc tax
inner join hr_accrual acc on acc.ID = tax.accrualID
...
and acc.flagsRec & 4096 = 0 and acc.flagsRec & 8192 = 0
and tax.taxLimitID1 is not null ) t
GROUP BY t.empTaxCodeType, t.taxCode, t.workPlace, t.limitedAccess, t.code, t.limit1, t.limit2, t.limit3
ORDER BY t.limitedAccess DESC
  `
  const store = UB.DataStore('hr_taxIndividAcc')
  store.runSQL(SQL,
    {
...
function getMilitaryTaxSum (orgID, period, employeeNumbers) {
  const sqlDialect = entityBaseService.getSQLDialect()
  const SQL = `
   SELECT 
      t.taxCode "taxCode",
   ...
   select 
      ...
      acc.paySum taxSum,
      ti.taxSum dtTaxSum,
      COALESCE(di.codeReport, di.code) code,
      (CASE WHEN (select ${sqlDialect.top} ep.workPlace from hr_employeePosition ep where ep.employeeNumberID = empNum.ID AND ep.dateFrom <= :dateTo: and ep.isActive = 1  
    and ep.mi_deleteDate >= '9999-12-31' order by ep.dateTo desc ${sqlDialect.limit}) = '3' THEN '3' ELSE '1' END) workPlace  
    FROM hr_accrual acc
    JOIN hr_payEl pe ON pe.ID = acc.payElID 
    ...
    LEFT JOIN hr_taxIndividAcc ti ON ti.accrualID = acc.ID
    LEFT join hr_dictTaxIndivid di on di.ID = ti.taxIndividID and di.mi_deleteDate >= '9999-12-31'
    where empNum.orgID = :orgID:
    ${employeeNumbers ? `and empNum.ID${entityBaseService.getInExpression('employeeNumbers')}` : ''} 
    and acc.periodCalcID = :periodID: AND m.code = '27' -- Военный сбор
    and acc.flagsRec & 4096 = 0 and acc.flagsRec & 8192 = 0
    AND empNum.empWorkPlace IS NULL 
    and empNum.mi_deleteDate >= '9999-12-31'
    ) t
    GROUP BY t.taxCode, t.workPlace, t.limitedAccess, t.code
    ORDER BY t.limitedAccess DESC
  `
...
function xmlExport ({ data, idx }) {
  ...
  correctRowNum({ DECLARBODY, tabIndex: 'T3' })
  ...
  // del Military data
  if (data.data.repCode !== 'J05104091') {

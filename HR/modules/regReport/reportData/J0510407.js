...
function prepareStructureReport () {
  ...
  data.DECLAR['$'] = {
    ...
    'xsi:noNamespaceSchemaLocation': 'J0510407.xsd'
  }
...
function getAllData (params, periodCalc) {
    ...
    if (params.contractorID) {
      ...
      payObligatoryDep.forEach(dep => {
        UB.Repository('hr_employeePositionSR')
          ...
          .where('dateFrom', '<=', periodCalc.dateTo)
          .where('dateTo', '>=', periodCalc.dateTo)
  ...
  militaryTaxSumList.forEach(mil => {
    const taxPayerData = taxPayersContainer.filter(o => o.taxCode === mil.taxCode)
...
function xmlExport ({ data, idx }) {
  ...
  // del Military data
  if (data.data.repCode !== 'J05104071') {

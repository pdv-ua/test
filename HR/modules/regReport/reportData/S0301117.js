...
function prepareStructureReport (data) {
  ...
  data.DECLAR['$'] = {
    ...
    'xsi:noNamespaceSchemaLocation': 'S0301117.xsd'
  }
...
function prepareDataSpecific ({ data, params }) {
  ...
  organiozations.forEach(orgID => {
    ...
    let firedEmpsBuld = UB.Repository('hr_employeePositionS')
      .attrs(['employeeNumberID', 'employeeNumberID.employeeID.sexType', 'employeeNumberID.parentEmpNumberID'])
      .where('[organizationID]', '=', orgID)
    ...
    const firedEmps = firedEmpsBuld.groupBy(['employeeNumberID', 'employeeNumberID.employeeID.sexType', 'employeeNumberID.parentEmpNumberID'])
      .selectAsObject({
  ...
  `, {
      ...
      DECLARBODY.A3050_1 = (DECLARBODY.A3050_1 || 0) + (fireOrders3050.length ? fireOrders3050[0].c_all : 0)
      DECLARBODY.A3050_2 = (DECLARBODY.A3050_2 || 0) + (fireOrders3050.length ? (fireOrders3050[0].c_women || 0) : 0)
    } else {
  ...
  `, {
      ...
      DECLARBODY.A3060_1 = (DECLARBODY.A3060_1 || 0) + (fireOrders3060.length ? fireOrders3060[0].c_all : 0)
      DECLARBODY.A3060_2 = (DECLARBODY.A3060_2 || 0) + (fireOrders3060.length ? (fireOrders3060[0].c_women || 0) : 0)
    } else {
      DECLARBODY.A3060_1 = (DECLARBODY.A3060_1 || 0)
      DECLARBODY.A3060_2 = (DECLARBODY.A3060_2 || 0)
    }

    const buldWorkingEmps = UB.Repository('hr_employeePositionS')
      .where('[organizationID]', '=', orgID)
      .where('[dateFrom]', '<=', params.dateTo)
      .where('[dateTo]', '>=', params.dateTo)
      .where('[workPlace]', '=', '1')
    ...
    const workingEmps = buldWorkingEmps.selectAsObject({
      'employeeNumberID.employeeID.sexType': 'sexType'
    })
    ...
    if (data.DECLAR.DECLARHEAD.PERIOD_MONTH === 12) {
      ...
      DECLARBODY.A3080_1 = (DECLARBODY.A3080_1 || 0) + partialTimeCount.c_all
      DECLARBODY.A3080_2 = (DECLARBODY.A3080_2 || 0) + partialTimeCount.c_women
      ...
      DECLARBODY.A3090_1 = (DECLARBODY.A3090_1 || 0) + timeSheets3090.c_all
      DECLARBODY.A3090_2 = (DECLARBODY.A3090_2 || 0) + timeSheets3090.c_women
      ...
      DECLARBODY.A3100_1 = (DECLARBODY.A3100_1 || 0) + timeSheets3100.c_all
      DECLARBODY.A3100_2 = (DECLARBODY.A3100_2 || 0) + timeSheets3100.c_women
    }
    ...
    DECLARBODY.A4080 = currencyService.gaussRound((DECLARBODY.A4080 || 0) + currencyService.gaussRound(notWorkedTime4080.c_hour))
    ...
    const notWorkedTime4090 = periodMonths.map(periodMonth => {
      return UB.Repository('tim_timeSheet')
        ...
        .attrs(['employeeNumberID.employeeID', 'SUM([planHour]-[factHour])'])
        .groupBy('employeeNumberID.employeeID')
        .selectAsObject({
          'employeeNumberID.employeeID': 'employeeID',
          'SUM([planHour]-[factHour])': 'c_hour'
        }).reduce((accum, row) => {
          accum.c_hour += row.c_hour
          accum.employeeIDs.push(row.employeeID)
          return accum
    ...
    DECLARBODY.A4090 = currencyService.gaussRound((DECLARBODY.A4090 || 0) + currencyService.gaussRound(notWorkedTime4090.c_hour))
    ...
    DECLARBODY.A4100 = currencyService.gaussRound((DECLARBODY.A4100 || 0) + currencyService.gaussRound(notWorkedTime4100.c_hour))
    DECLARBODY.B4100 = (DECLARBODY.B4100 || 0) + (new Set(notWorkedTime4100.employeeIDs)).size

    // Part III

    function part3Alg (mainPayElIDs, addPayElIDs, excludePayElIDs) {
      ...
      return currencyService.gaussRound(paySum / 100) / 10
    }

    DECLARBODY.A5020 = (DECLARBODY.A5020 || 0) + part3Alg(reportParams.FOZPIDs, null, reportParams.notFOPS03IDs)
    DECLARBODY.A5030 = (DECLARBODY.A5030 || 0) + part3Alg(reportParams.FDZPIDs, null, reportParams.notFOPS03IDs)
    DECLARBODY.A5040 = (DECLARBODY.A5040 || 0) + part3Alg(reportParams.FDZPIDs, reportParams['5040IDs'], reportParams.notFOPS03IDs)
    DECLARBODY.A5050 = (DECLARBODY.A5050 || 0) + part3Alg(reportParams.FDZPIDs, reportParams['5050IDs'], reportParams.notFOPS03IDs)
    DECLARBODY.A5051 = (DECLARBODY.A5051 || 0) + part3Alg(reportParams.FDZPIDs, reportParams['5051IDs'], reportParams.notFOPS03IDs)
    DECLARBODY.A5052 = (DECLARBODY.A5052 || 0) + part3Alg(reportParams.FDZPIDs, reportParams['5052IDs'], reportParams.notFOPS03IDs)
    DECLARBODY.A5060 = (DECLARBODY.A5060 || 0) + part3Alg(reportParams.ZKVIDs, null, reportParams.notFOPS03IDs)
    DECLARBODY.A5070 = (DECLARBODY.A5070 || 0) + part3Alg(reportParams.ZKVIDs, reportParams['5070IDs'], reportParams.notFOPS03IDs)
    DECLARBODY.A5080 = (DECLARBODY.A5080 || 0) + part3Alg(reportParams.ZKVIDs, reportParams['5080IDs'], reportParams.notFOPS03IDs)
    DECLARBODY.A5090 = (DECLARBODY.A5090 || 0) + part3Alg(reportParams.FDZPIDs.concat(reportParams.ZKVIDs), reportParams['5090IDs'], reportParams.notFOPS03IDs)
    ...
    const hourTotalsByEmployee = workedTimeData.reduce((accum, row) => {
      ...
      if (!employeeHours) {
        employeeHours = accum[row.employeeNumberID] = {
    ...
    hourTotals.forEach(hourTotals => {
      hourTotals.timePerc = currencyService.gaussRound((hourTotals.fullHour + hourTotals.shortHour + hourTotals.absenseHour) / hourTotals.planHour * 100)
    })
    ...
    DECLARBODY.A6020 = (DECLARBODY.A6020 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[0] && (salaryRanges[0].valueFrom <= row.paySum) && (row.paySum < salaryRanges[0].valueTo)).length
    DECLARBODY.A6030 = (DECLARBODY.A6030 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[1] && (salaryRanges[1].valueFrom <= row.paySum) && (row.paySum < salaryRanges[1].valueTo)).length
    DECLARBODY.A6040 = (DECLARBODY.A6040 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[2] && (salaryRanges[2].valueFrom <= row.paySum) && (row.paySum < salaryRanges[2].valueTo)).length
    DECLARBODY.A6050 = (DECLARBODY.A6050 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[3] && (salaryRanges[3].valueFrom <= row.paySum) && (row.paySum < salaryRanges[3].valueTo)).length
    DECLARBODY.A6060 = (DECLARBODY.A6060 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[4] && (salaryRanges[4].valueFrom <= row.paySum) && (row.paySum < salaryRanges[4].valueTo)).length
    DECLARBODY.A6070 = (DECLARBODY.A6070 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[5] && (salaryRanges[5].valueFrom <= row.paySum) && (row.paySum < salaryRanges[5].valueTo)).length
    DECLARBODY.A6080 = (DECLARBODY.A6080 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[6] && (salaryRanges[6].valueFrom <= row.paySum) && (row.paySum < salaryRanges[6].valueTo)).length
    DECLARBODY.A6090 = (DECLARBODY.A6090 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[7] && (salaryRanges[7].valueFrom <= row.paySum) && (row.paySum < salaryRanges[7].valueTo)).length
    DECLARBODY.A6100 = (DECLARBODY.A6100 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[8] && (salaryRanges[8].valueFrom <= row.paySum) && (row.paySum < salaryRanges[8].valueTo)).length
    DECLARBODY.A6110 = (DECLARBODY.A6110 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[9] && (salaryRanges[9].valueFrom <= row.paySum) && (row.paySum < salaryRanges[9].valueTo)).length
    DECLARBODY.A6120 = (DECLARBODY.A6120 || 0) + hourTotals.filter(row => row.timePerc >= 50 && salaryRanges[10] && (salaryRanges[10].valueFrom <= row.paySum) && (row.paySum < salaryRanges[10].valueTo)).length
    DECLARBODY.A6130 = (DECLARBODY.A6130 || 0) + hourTotals.filter(row => row.timePerc >= 100).length
    DECLARBODY.A6140 = (DECLARBODY.A6140 || 0) + hourTotals.filter(row => row.timePerc >= 100 && row.paySum < minSalarySum).length

    DECLARBODY.A6150 = (DECLARBODY.A6150 || 0) + (UB.Repository('tim_timeSheet')
    ...
    DECLARBODY.B7010 = (DECLARBODY.B7010 || 0) + currencyService.gaussRound(part5Data['7010'].paySum / 100) / 10
    DECLARBODY.B7030 = (DECLARBODY.B7030 || 0) + currencyService.gaussRound(part5Data['7030'].paySum / 100) / 10
    DECLARBODY.B7040 = (DECLARBODY.B7040 || 0) + currencyService.gaussRound(part5Data['7040'].paySum / 100) / 10
    DECLARBODY.C7010 = currencyService.gaussRound((DECLARBODY.C7010 || 0) + currencyService.gaussRound(part5Data['7010'].factHour))
  })
...
const cellFormats = [
  {
    names: ['FIRM_ADR', 'FIRM_ADR_FIZ', 'FIRM_EDRPOU', 'FIRM_FAXORG', 'FIRM_NAME', 'FIRM_SPATO', 'FIRM_KVED', 'SPATO', 'KVED', 'REP_PERNM', 'OBL', 'RAY', 'TER_STRUK', 'VIK', 'MY_DATE', 'VIK_RUK', 'VIK_TEL', 'VIK_EMAIL', 'N9', 'N11', 'N1', 'N12', 'K12'],
    format: {
      type: 'string',
      nillable: true
    }

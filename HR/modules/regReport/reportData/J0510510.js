...
const allBodyAttrNames = [
  'HTIN', 'HTIN1', 'HNAME', 'HZY', 'HZKV', 'HZNUM1', 'HZN', 'HZU', 'HZ', 'HZB', 'HZS', 'HZD', 'HFIL', 'H01', 'H02', 'HNUM1',
...
function prepareStructureReport (data) {
  ...
  data.DECLAR['$'] = {
    ...
    'xsi:noNamespaceSchemaLocation': 'J0510510.xsd'
  }
...
function prepareDataSpecific ({ data, params }) {
  ...
  DECLARBODY.HZB = params.FORM_TYPE === 'HZB' ? '1' : '0'
  DECLARBODY.HZS = params.FORM_TYPE === 'HZS' ? '1' : '0'
  DECLARBODY.HZD = params.FORM_TYPE === 'HZD' ? '1' : '0'
    ...
    periods.forEach((period) => {
      ...
      let empPosDatas = UB.Repository('hr_employeePositionS')
        .attrs(['employeeNumberID', 'employeeID', 'dateFrom', 'dateTo', 'isActive', 'orderID', 'changeOrderID', 'departmentID', 'positionID', 'dictPositionID',
          ...
          'changeOrderNumber', 'changeOrderDate', 'dictStaffCatID.accCategory', 'ID', 'employeeNumberID.parentEmpNumberID', 'employeeNumberID.childEmpNumberID'])
        .where('[organizationID]', '=', orgID)
      ...
      empPosDatas.forEach((row, index) => {
        ...
        if (!row['changeOrderID.orderDate'] && row.changeOrderDate) {
          row['changeOrderID.orderDate'] = row.changeOrderDate
        }
        if (!row['changeOrderID.orderNumber'] && row.changeOrderNumber) {
          row['changeOrderID.orderNumber'] = row.changeOrderNumber
        }
        if (row.dateTo >= period.dateFrom && row['dictStaffCatID.accCategory'] !== '7') {
          if (params.isInclude5 && ((row['orderID.empOrderType'] !== 'APPOINT' && row['changeOrderID.empOrderType'] !== 'DISM' && row['orderID.empOrderType'] !== 'CHGSALARY' &&
            ...
            if (params.isIncludeTwoRow5 && row.dateFrom > params.dateFrom) {
              const priorRow = empPosDatas[index - 1]
      ...
      const empAccrDatas = UB.Repository('hr_employeeAccrual')
        .attrs(['employeeNumberID', 'employeeID', 'dateFrom', 'dateTo', 'orderID', 'changeOrderID', 'payElID.methodID.code',
          ...
          'orderID.orderDate', 'orderID.orderNumber', 'orderID.empOrderType', 'changeOrderID.orderDate', 'changeOrderID.orderNumber', 'changeOrderID.empOrderType',
          'orderNumber', 'orderDate', 'changeOrderNumber', 'changeOrderDate'
        ])
      ...
      empAccrDatas.forEach(row => {
        row.dateFrom = dateService.shiftDate(row.dateFrom)
        row.dateTo = dateService.shiftDate(row.dateTo)
        if (!row['orderID.orderDate'] && row.orderDate) {
          row['orderID.orderDate'] = row.orderDate
        }
        if (!row['orderID.orderNumber'] && row.orderNumber) {
  ...
  const uniqueReportData = _.uniqWith(reportData, _.isEqual)
  uniqueReportData.sort((a, b) =>
...
const cellFormats = [
  ...
  {
    names: [ 'HZ', 'HZY', 'HZKV', 'HNUM1', 'HZB', 'HZS', 'HZD', 'HZN', 'HZU' ],
    format: {
      type: 'number',
      nillable: false,
      precision: 0

...
me.details = [
  {
    detailName: 'empOrderDowntimeListDet',
    entityName: 'hr_empOrderDowntimeListDet',
    docIDName: 'paraID',
...
function getDescription (dateFrom, dateTo) {
  ...
  return UB.i18n(`Простій з {0}{1}`, dateFromStr, dateToStr)
}
...
function syncDates (ctx) {
  ...
  if (dateFrom !== undefined || dateTo !== undefined) {
    const ds = UB.DataStore('hr_empOrderDowntimeListDet')
    const detList = UB.Repository('hr_empOrderDowntimeListDet')
      .attrs(['ID'])
...
me.loadEmployeeList = function (ctx) {
  ...
  const ds = UB.DataStore('hr_empOrderDowntimeListDet')
  const existing = UB.Repository('hr_empOrderDowntimeListDet')
    .attrs('ID')

...
me.search = function (ctx) {
  ...
  let sqlBuilder = {
    text: `
      ...
      JOIN hr_payFund pf ON pf.ID = af.payFundID AND pf.isRecSum = 0
JOIN hr_employeeNumber en on af.employeeNumberID = en.ID 
JOIN hr_employee emp on en.employeeID = emp.ID 
LEFT JOIN hr_employeePosition ep on en.ID = ep.employeeNumberID 
  and ep.organizationID = en.orgID 
  and :periodDateTo: between ep.dateFrom and ep.dateTo
  and ep.isActive = 1 
  and ep.mi_deleteDate >= '9999-12-31' 
JOIN hr_dictPeriod dp on af.periodSalaryID = dp.ID
      {2} 
      GROUP BY af.employeeNumberID, af.periodSalaryID, ep.positionID, en.tabNum, emp.lastName, emp.firstName, emp.middleName, dp.name, ep.factPosName, ep.dictPositionID  
      HAVING SUM(af.sourceSum) > SUM(af.baseSum - COALESCE(af.addMinSum, 0))
      {3} {4}
    `,
    clauses: {},
    aliases: {
      ...
      maxSum: { field: `sum(af.baseSum - COALESCE(af.addMinSum, 0))` },
      greater: { field: `sum(af.sourceSum) - sum(af.baseSum - COALESCE(af.addMinSum, 0))` }
    },
    params: {}
  }
...
and (ep.dateFrom = (select ${sqlDialect.top} ep2.dateFrom from hr_employeePosition ep2 
 where ep2.mi_deleteDate >= '9999-12-31' and ep2.isActive = 1
 and :periodDateTo: between ep2.dateFrom and ep2.dateTo
 and ep2.employeeNumberID = en.ID  
 and ep2.isActive = 1 
 and ep2.organizationID = en.orgID 
 ${kindClause2}
 order by ep2.dateFrom desc ${sqlDialect.limit} 
  ) OR ep.dateFrom IS NULL)
 ${kindClause} 

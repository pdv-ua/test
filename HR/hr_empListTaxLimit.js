...
me.search = function (ctx) {
  ...
  let sqlBuilder = {
    text: `
      select {0} {1}
      FROM hr_employeeTaxLimit et 
JOIN hr_employeeNumber en on et.employeeNumberID = en.ID 
...
JOIN hr_taxLimit tl on et.taxLimitID = tl.ID 
      {2} {3} {4}
    `,
    clauses: {},
    aliases: {
      ...
      taxLimitName: { field: `(CONCAT(tl.name,' (',tl.size,')'))` },
      taxLimitType: { field: `
      (select ${sqlDialect.top} 
       CASE 
       WHEN enm.code = '2'
       THEN CONCAT(enm.name,' (',et.amountChild,')') 
       ELSE enm.name 
       END typ 
       from  ubm_enum enm 
       where enm.code = tl.taxLimitType 
       and enm.eGroup = 'HR_TAXLIMIT_TYPE' 
  ...
  sqlBuilder.clauses.whereParams.onDate = dateService.shiftDate(mParams.onDate)

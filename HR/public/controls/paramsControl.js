...
Ext.define('HR.controls.paramsControl', {
  extend: 'Ext.panel.Panel',
  alias: 'widget.paramsControl',

  layout: 'fit',
  payElEntryType: ['PAYMENT'],
  initComponent: function () {
    const me = this
    const hasListParamAccess = true
                                ...
                                switch (me.attr.reportValuesGrid.store.ubRequest.tableNameForMapping) {
                                  case 'hr_payEl':
                                    $App.doCommand({
                                      ...
                                      cmpInitConfig: {
                                        withPeriod: true,
                                        payElEntryType: me.payElEntryType,
    ...
    me.attr = {
      listParamGrid: me.queryById('listParamGrid'),
    ...
    if (!hasListParamAccess) {
      me.attr.listParamGrid.setReadOnly(true)
    ...
    function onParamSelect (selectedRow) {
      ...
      if (!selectedRow || !selectedRow.get('ID')) {
        ...
        me.attr.currentParamFld.setValue(selectedRow.get('fullName'))
        me.attr.reportValuesGrid.parentContext = {
          listParamID: selectedRow.get('ID'),
          listParamCode: selectedRow.get('code'),
          orgID: orgID
        }
        store.ubRequest.tableNameForMapping = selectedRow.get('tableName')
        store.ubRequest.whereList.byParam = {
          expression: '[listParamID]',
          condition: 'equal',
    ...
    me.attr.listParamGrid.on('selectionchange', (selectionModel, selected) => {
      onParamSelect(selected && selected[0])
    })
    me.attr.listParamGrid.store.on('load', () => {
      if (!me.attr.listParamGrid.getSelectionModel().getSelection().length && me.attr.listParamGrid.store.count()) {
        onParamSelect(me.attr.listParamGrid.store.getAt(0))
      }
    })

    me.attr.listParamGrid.on('changeData', () => {
      me.attr.listParamGrid.store.load()
    })

    me.setListParamIDs = (listParamIDs) => {
      if (listParamIDs) {
        const store = me.attr.listParamGrid.store
        store.ubRequest.whereList.IDs = {
          expression: '[ID]',
          condition: 'in',
          value: listParamIDs
        }
    ...
    me.on('afterrender', () => {
      ...
      if (me.readOnly) {
       ...
       // me.down('[itemId=createParam]').hide()
        const tb = me.attr.listParamGrid.down('toolbar')
        tb && tb.hide()
      }
    })
  }

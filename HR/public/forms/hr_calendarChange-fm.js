...
function onAfterSave () {
  ...
  if (parentForm) {
    parentForm.onPeriodChangeChange(parentForm)
  }
}
...
function showChangeForm (me, grid, permType, attrName, sourceData) {
  UB.Repository('hr_calendarChangeDt')
    .attrs(['ID', attrName])
    .where('calendarChangeID', '=', me.instanceID)
    .selectAsObject({
      [attrName]: 'value'
    })
    .then(result => {
          ...
          onSelectData: (data) => {
            if (data.remove.length || data.add.length) {
              $App.connection.run({
                ...
                method: 'updateCalendarChangeDt',
                calendarChangeID: me.instanceID,

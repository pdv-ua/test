...
const periodMetods = ['45', '46', '65', '206']
...
function addBaseActions () {
  ...
  me.actions.filterButtonAction = new Ext.Action({
    ...
    handler: function (btn) {
      const localStorageData = JSON.parse(UB.core.UBLocalStorageManager.getItem(`hr_orderRegistryPremium_${me.attr.payElID.getFieldValue('methodID.code')}_${appAC.globalOrganization()}`) || '{}')
      me.isFilter = !me.isFilter
      if (!localStorageData) {
        UB.core.UBLocalStorageManager.setItem(`hr_orderRegistryPremium_${me.attr.payElID.getFieldValue('methodID.code')}_${appAC.globalOrganization()}`, {
  ...
  me.actions.exportExcel = new Ext.Action({
    ...
    handler: function (btn) {
      ...
      columns.forEach((el, idx) => {
        ...
        if (['paySumAccrual', 'paySumOff', 'paySum'].includes(el[0])) {
      ...
      if (isIncludeSummary) {
        ...
        while (idx < columns.length) {
          if (!['paySumAccrual', 'paySumOff', 'paySum'].includes(columns[idx][0])) {
        ...
        columns.forEach((el, idx) => {
          switch (el[0]) {
            ...
            case 'paySumOff':
              addSumText(idx)
              dataRow.push({ column: idx, value: paySumOff, style: styleSummaryCol })
              break
            case 'paySum':
              addSumText(idx)
...
function updateIndividuslRate (me, reCalcRate) {
  const store = me.attr.orderRegistryDt.getStore()
  const allRecords = store.snapshot || store.data
  const empIDs = []
  ...
  if (empIDs.length) {
    const dateFrom = periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code'))
      ? me.attr.periodFromAvg.getFieldValue('dateFrom') : me.attr.periodSalaryID.getFieldValue('dateFrom')
    const dateTo = periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code'))
      ? me.attr.periodToAvg.getFieldValue('dateTo') : me.attr.periodSalaryID.getFieldValue('dateTo')
    UB.Repository('hr_employeeAccrual')
      ...
      .selectAsObject().then(payPerms => {
        ...
        allRecords.each(record => {
          ...
          const payPerm = payPerms.filter(o => o.employeeNumberID === employeeNumberID)
          payPerm.forEach(empRate => {
            if (empRate.accrualRate) {
              record.set('additionalRate', empRate.accrualRate)
              record.set('baseSum', me.attr.baseSum.getValue())
            } else {
              record.set('baseSum', empRate.accrualSum)
    ...
    case 'rate':
      if (value !== null) {
        ...
        if (me.attr.payElID.getFieldValue('isIndividualRate')) {
          runRecalc = false
          me.updateIndividuslRate(me, reCalcRate)
        }
        // accrualSum
      } else {
        allRecords.each(record => {
...
function reCalc (me, clearFundSource, reCalcRate) {
  ...
  me.calcRegistryPremium(me, params)
}
...
function onFormDataReady () {
  ...
  const localData = JSON.parse(UB.core.UBLocalStorageManager.getItem(`hr_orderRegistryPremium_${me.record.get('payElID.methodID.code')}_${appAC.globalOrganization()}`) || '{}')
  if (me.isNewInstance) {
  ...
  // AC.viewUtils.setFilterValue(me.attr.payElID, { 'methodID.methodGroupID.code': 3 })
  ...
  const readOnlyAttr = ['orderDate', 'docNumber', 'periodFromAvg', 'periodToAvg']
...
function setDescription (me) {
  me.attr.name.setValue(`${UB.i18n('Премія')}${me.attr.dictMultiGroupID.getValue()
...
function onControlChanged (me, field, value) {
  if (me.attr.payElID.getFieldValue('isIndividualRate') &&
    ['payElID', 'periodSalaryID', 'periodFromAvg', 'periodToAvg'].includes(field.name)) {
...
function setAvgPeriod (me) {
  const methodCode = me.attr.payElID.getFieldValue('methodID.code')
  const onDate = me.attr.periodSalaryID.getFieldValue('dateFrom')
  let dateFromAvg = null
  let dateToAvg = null
  ...
  if (periodMetods.includes(methodCode) && onDate) {
    const month = onDate.getMonth()
    switch (me.attr.payElID.getFieldValue('methodID.code')) {
      case '65':
      case '206':
        dateFromAvg = AC.dateService.addMonths(onDate, -1)
        dateToAvg = AC.dateService.lastDayOfMonth(dateFromAvg)
        break
      case '45':
        if ([2, 5, 8, 11].includes(month)) {
          dateFromAvg = AC.dateService.addMonths(onDate, -2)
          dateToAvg = AC.dateService.lastDayOfMonth(onDate)
        } else {
          dateFromAvg = AC.dateService.getQuarterDates(onDate.getFullYear(), AC.dateService.getQuarter(onDate) - 1).dateFrom
          dateToAvg = AC.dateService.lastDayOfMonth(AC.dateService.addMonths(dateFromAvg, 2))
        }
        break
      case '46':
        if (month === 11) {
          dateFromAvg = AC.dateService.firstDayOfYear(onDate)
        } else {
          dateFromAvg = AC.dateService.firstDayOfYear(AC.dateService.addYears(onDate, -1))
        }
        dateToAvg = AC.dateService.lastDayOfMonth(AC.dateService.addMonths(dateFromAvg, 11))
        break
    }
    $App.connection.run({
      entity: 'hr_orderRegistry',
      method: 'getPeriodsOnDates',
      params: JSON.stringify({ orgID: me.record.get('organizationID'), onDateFrom: AC.dateService.shiftDate(dateFromAvg), onDateTo: AC.dateService.shiftDate(dateToAvg) })
    }).then(response => {
      const data = JSON.parse(response.resultData)
      me.attr.periodFromAvg.setValue(data.periodFrom.ID)
      me.attr.periodToAvg.setValue(data.periodTo.ID)
      me.record.set('periodFromAvg', data.periodFrom.ID)
      me.record.set('periodToAvg', data.periodTo.ID)
      me.attr.periodFromAvg.resumeEvents()
      me.attr.periodToAvg.resumeEvents()
    })
  } else {
    me.attr.periodFromAvg.setValue()
    me.attr.periodToAvg.setValue()
    me.record.set('periodFromAvg', null)
    me.record.set('periodToAvg', null)
    me.attr.periodFromAvg.resumeEvents()
    me.attr.periodToAvg.resumeEvents()
  }
...
function onGridEdit (me, context) {
  ...
  function addRecord () {
    const params = {
      ...
      payElParams: [{
        ...
        payElID: me.attr.payElID.getValue(),
        additionalRate: context.record.get('additionalRate'),
        KPI: context.record.get('KPI'),
  ...
  function individualRate () {
    ...
    UB.Repository('hr_employeeAccrual')
      ...
      .selectSingle().then(payPerms => {
        if (payPerms) {
          if (payPerms.accrualRate) {
            context.record.set('additionalRate', payPerms.accrualRate)
          } else {
            context.record.set('flagsFix', context.record.get('flagsFix') | me.attr.baseSum.flagsFix)
            context.record.set('additionalRate', 100)
            context.record.set('baseSum', payPerms.accrualSum)
          }
        }
        addRecord()
      })
  ...
  switch (context.column.field.name) {
    ...
    case 'rate':
      if (context.value !== context.originalValue) {
        // context.record.set('paySum', AC.currencyService.round(context.record.get('baseSum') * context.record.get('rate') / 100))
        let flagFix = context.record.get('flagsFix')
        flagFix &= ~(1 << 21)
        flagFix |= (1 << 9)
        context.record.set('flagsFix', flagFix)
    ...
    case 'paySum':
      if (context.value !== context.originalValue) {
        // context.record.set('rate', context.record.get('baseSum') ? AC.currencyService.round(context.record.get('paySum') / context.record.get('baseSum') * 100) : 0)
        let flagFix = context.record.get('flagsFix')
        flagFix |= (1 << 1)
        context.record.set('flagsFix', flagFix)
        addRecord()
...
function setEmployeeNumbers (me) {
  ...
  $App.doCommand({
    ...
    cmpInitConfig: {
      ...
      parentCode: 'hr_orderRegistryPremium',
      defaultValues: {
        periodFromID,
        periodToID,
        depID: me.record.get('departmentID'),
              ...
              .selectAsObject().then(payPerms => {
                data.forEach(row => {
                  ...
                  payPerm.forEach(empRate => {
                    ...
                    newRow.KPI = getKpi(row.employeeNumberID, dateFrom)
                    ...
                    if (empRate.accrualRate) {
                      ...
                      newRow.flagsFix = newRow.flagsFix | me.attr.baseSum.flagsFix
                    }
                    newRow.rate = me.attr.rate.getValue()
                    ...
                    if (periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code'))) {
                      ...
                      newRow.dateFrom = AC.dateService.shiftDate(Math.max(dateFrom, AC.dateService.shiftDate(empRate.dateFrom)))
                      newRow.dateTo = AC.dateService.shiftDate(Math.min(dateTo, AC.dateService.shiftDate(empRate.dateTo)))
                    }
        ...
        const setData = () => {
          ...
          data.forEach(row => {
            if (!store.findRecord('employeeNumberID', row.employeeNumberID)) {
              ...
              row['depName'] = row['depCaption']
              delete row['depCaption']
              ...
              if (periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code'))) {
                row.dateFromAvg = me.attr.periodFromAvg.getFieldValue('dateFrom')
                row.dateToAvg = me.attr.periodToAvg.getFieldValue('dateTo')
              }
        ...
        $App.connection.run({
          ...
          if (me.attr.orderRegistryDt.getStore().count()) {
            $App.dialogYesNo('Попередження', UB.i18n('Видалити раніше внесені нарахування?'))
              .then(res => {
                if (res) {
                  me.attr.orderRegistryDt.getStore().on('clear', me.attr.payElID.getFieldValue('isIndividualRate') ? setIndividuslRate : setData)
                  me.attr.orderRegistryDt.removeAll()
                } else {
                  if (me.attr.payElID.getFieldValue('isIndividualRate')) {
                    setIndividuslRate()
...
function calcRegistryPremium (me, params) {
  let scrollPosition
  if (!!params.payElParams.length && !!params.periodSalaryID && !!me.attr.payElID.getValue()) {
    me.setLoading(true)
    $App.connection.run({
      entity: 'hr_orderRegistry',
      method: 'calcRegistryPremium',
      params: JSON.stringify(params)
    }).then(response => {
      let scrollTop = me.attr.orderRegistryDt.getView().getEl().dom.scrollTop
      const data = JSON.parse(response.resultData)
      const store = me.attr.orderRegistryDt.getStore()
      me.attr.orderRegistryDt.suspendEvents()
      store.suspendEvents()
      ...
      if (data.length === 1) {
        me.attr.orderRegistryDt.getView().getEl().dom.scrollTop = scrollTop
      }
      me.setIsDirty(true)
      me.setLoading(false)
    }, function (err) {
...
function setVisibleKpi (me) {
  const useKPI = !!me.attr.payElID.getFieldValue('useKPI')
  me.attr.baseSum[useKPI ? 'hide' : 'show']()
  me.attr.rate[useKPI ? 'hide' : 'show']()
  AC.gridUtils.setGridColumnVisible(me.attr.orderRegistryDt, ['KPI'], useKPI)
}

// TODO
function getKpi (employeeNumberID, onDate) {
  return null
}
...
function loadCsvFile (ctrl) {
  const me = ctrl.up('form')
  let dataClear = false
  if (me.isNewInstance) {
    $App.dialogInfo(`Необхідно спочатку зберегти документ`)
    return
  }

  if (me.attr.orderRegistryDt.getStore().count()) {
    $App.dialogYesNo('Попередження', UB.i18n('В документі нарахування вже є працівники! Видалити?')).then(isAgree => {
      if (isAgree) {
        me.attr.orderRegistryDt.removeAll()
        loadCsvFileData(ctrl, dataClear = true)
      }
    })
  } else {
    loadCsvFileData(ctrl, dataClear)
  }
}

function loadCsvFileData (ctrl, dataClear) {
  const me = ctrl.up('form')
  const entityName = 'hr_orderRegistryDt'
  const entityAttrs = AC.entityUtils.getAttributes(entityName)
    ...
    upLoad: function (btn) {
      ...
      UB.connection.post('loadImportDataEx', file, {
        ...
        const params = {
          ...
          orderParams: {
            payElID: me.attr.payElID.getValue(),
            isIndividualRate: me.attr.payElID.getFieldValue('isIndividualRate'),
            orderRate: me.attr.rate.getValue(),
            baseSum: me.attr.baseSum.getValue(),
            orderDate: me.attr.orderDate.getValue(),
            orderID: me.instanceID,
            dateFrom: periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code')) ? me.attr.periodFromAvg.getFieldValue('dateFrom') : me.attr.periodSalaryID.getFieldValue('dateFrom'),
            dateTo: periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code')) ? me.attr.periodToAvg.getFieldValue('dateTo') : me.attr.periodSalaryID.getFieldValue('dateTo')
          },
          payElParams: parsedData
        }
        ...
        if (params.payElParams.length && params.periodSalaryID && me.attr.payElID.getValue()) {
          ...
          $App.connection.run({
            ...
            data.result.forEach(row => {
              row.dateFrom = me.attr.periodSalaryID.getFieldValue('dateFrom')
              row.dateTo = me.attr.periodSalaryID.getFieldValue('dateTo')
              row.dateFromAvg = (periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code')) ? me.attr.periodFromAvg.getFieldValue('dateFrom') : null)
              row.dateToAvg = (periodMetods.includes(me.attr.payElID.getFieldValue('methodID.code')) ? me.attr.periodToAvg.getFieldValue('dateTo') : null)
            })
            if (data.result.length) {
              ...
              me.attr.orderRegistryDt.getStore().on('add', bind)
              me.attr.orderRegistryDt.getStore().insert(me.attr.orderRegistryDt.getStore().data.length, data.result)
            } else {
              me.setLoading(false)
            }
            $App.doCommand({
              cmdType: 'showForm',
              formCode: 'hr_importResult',
              customParams: data.resultDownload
            })
          }, function (err) {
            me.setLoading(false)
            throw err

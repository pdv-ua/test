/* global AC HR $App _ appAC Ext UB appHR */
exports.formCode = {
  initComponentStart,
  ...
  switch (ctrl.name) {
    case 'dateFrom':
      AC.viewUtils.setWhereListProperty(me.attr.employeePositionID, [
        ...
        ['dateTo', '>=', me.record.get('orderDate') || me.record.get('dateFrom'), 'dateTo'],
        ['dateFrom', '<=', me.record.get('orderDate') || me.record.get('dateFrom'), 'dateFrom'],
      ...
      if (value && ctrl.calcValue !== value) {
        ...
        calcEasyWork(me, true, true, ctrl.name)
      }
      break
    case 'dateFromAvg' :
    case 'dateToAvg' :
...
function onFormDataReady () {
  ...
  me.actions.calcEasyWork.setDisabled(me.record.get('orderState') === 'POSTED')
  ...
  if (!me.isNewInstance) {
    ...
    me.attr.calendarDayCount.setReadOnly(true)
    me.attr.orderDate.setValue(null)
  }
}
...
function onControlChanged (me, field, value) {
  switch (field.name) {
    ...
    case 'payElID':
      ...
      break
    case 'dateFrom':
      me.attr.dateTo.setReadOnly(!value || !AC.dateService.isValid(value))
      me.attr.calendarDayCount.setReadOnly(!value || !AC.dateService.isValid(value))
      if (!value || !AC.dateService.isValid(value)) {
        me.attr.calendarDayCount.setValue(null)
        me.attr.dateTo.setValue(null)
      }
      AC.viewUtils.setWhereListProperty(me.attr.employeePositionID, [
        ['dateTo', '>=', me.record.get('orderDate') || me.attr.dateFrom.getValue(), 'dateTo'],
        ['dateFrom', '<=', me.record.get('orderDate') || me.attr.dateFrom.getValue(), 'dateFrom']
      ], ['(([dateTo] AND [dateFrom]) OR ([dismDateTo] AND [dismNumDate]))'], ['clearValue', 'clearStore'])
      break
    case 'calendarDayCount':
      if (value) {
        me.attr.dateTo.setValue(AC.dateService.addDays(me.attr.dateFrom.getValue(), value - 1))
      } else {
        me.attr.dateTo.setValue(null)
      }
      break
    case 'dateTo':
      if (value && AC.dateService.isValid(value)) {
        me.attr.calendarDayCount.setValue(AC.dateService.dayDiff(me.attr.dateFrom.getValue(), value))
      } else {
        me.attr.calendarDayCount.setValue(null)
      }
      break
...
function calcEasyWork (me, clear, clearAvg, ctrlName) {
  ...
  if (me.attr.calendarDayCount.getValue()) {
    me.attr.dateTo.calcValue = AC.dateService.addDays(me.attr.dateFrom.getValue(), me.attr.calendarDayCount.getValue() - 1)
    me.attr.dateTo.setValue(me.attr.dateTo.calcValue)
  } else if (me.attr.dateTo.getValue()) {
    me.attr.calendarDayCount.calcValue = AC.dateService.dayDiff(me.attr.dateFrom.getValue(), me.attr.dateTo.getValue())
...
function addBaseActions () {
  ...
  if (!me.actions.calcEasyWork) {
    me.actions.calcEasyWork = new Ext.Action({

...
function onFormDataReady () {
  ...
  if (me.isNewInstance) {
    me.setTitle(`${UB.i18n('Поновлення на посаді (оплата за середнім заробітком)')} ${me.record.get('orderNumber')} (${UB.i18n('Створення')})`)
  } else {
    me.setTitle(`${UB.i18n('Поновлення на посаді (оплата за середнім заробітком)')} ${me.record.get('orderNumber')}`)
  }

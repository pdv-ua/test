...
function onFormDataReady () {
  ...
  const readOnlyAttr = ['orderDate', 'orderNumber', 'employeePositionID', 'dateFrom', 'calendarDayCount', 'dayCount', 'dateTo', 'parentID']
...
function setControlState (me) {
  const visible = me.attr.payElID.getFieldValue('methodID.code') === '68'
  me.attr.parentID.setVisible(visible)
...
function setParentCtrlFilter (me, clearValue) {
  const filterParams = []
  const employeeNumberID = me.attr.employeePositionID.getFieldValue('employeeNumberID') || 0
  filterParams.push(['employeeNumberID', '=', employeeNumberID])
  filterParams.push(['payElID.methodID.code', '=', '68'])
...
function calcAvgPay (me, clear, clearAvg, ctrlName) {
  ...
  if (!ctrlName) {
    ctrlName = 'dateTo' // me.attr.dayCount.getValue() ? 'dayCount' : 'dateTo'
  }
  ...
  const params = {
    ...
    hourCount: me.attr.hourCount.getValue(),
    ...
    maxDateTo: me.attr.employeePositionID.getFieldValue('employeeNumberID.dateTo'),
    uiRecalc: true,
    accruals: [],
    accrualsAvg: []
  }
  ...
  $App.connection.run({
    ...
    me.attr.hourCount.calcValue = data.hourCount
    me.attr.hourCount.setValue(me.attr.hourCount.calcValue)
...
function addIntComb (empType = '1') {
  ...
  $App.connection.run({
    ...
    if (mParams.result) {
      ...
      $App.dialogInfo(msg.join('<br/>'), UB.i18n('Увага'))
    }
  }, err => {
    me.setLoading(false)
    throw err

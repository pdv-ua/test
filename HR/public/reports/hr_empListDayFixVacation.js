...
exports.reportCode = {
  ...
  getReportData: async function (reportParams) {
    ...
    let result = {
      ...
      colSpan: 5 + (showAddDescrPerson ? 1 : 0) + (useActualPositionName ? 1 : 0) + (reportParams.showDetails ? 1 : 0),
      tableWidth: 1070 + (showAddDescrPerson ? 150 : 0) + (useActualPositionName ? 200 : 0) + (reportParams.showDetails ? 150 : 0),
      orgUnits: [],
      positionCategory: '',
...
*/
    const fieldList = ['posName', 'posID', 'tabNum', 'fullFIO', 'dayFix', 'workPlace', 'depID', 'organizationID']
    if (showAddDescrPerson) fieldList.push('addDescrPerson')
    if (useActualPositionName) fieldList.push('actualPositionName')
    if (reportParams.showDetails) fieldList.push('periodValue')
    const empVacData = await $App.connection.run({
      ...
      // showOverUsedVac: reportParams.showOverUsedVac,
      showDetails: reportParams.showDetails,
      showFixDays: true
    })
    ...
    if (empData && empData.length) {
      empData = empData.filter(el => (el.dayFix || 0) > 0)
    }
    ...
    const tree = me.generateData(orgs, reportParams.departmentID || reportParams.organizationID, orgStruct, empData, result.colSpan, showAddDescrPerson, useActualPositionName, reportParams.showDetails)
    result.data = tree && tree.data ? tree.data : []

    return result
  },
  generateData: function (orgs, itemID, orgStruct, empVac, colSpan, showAddDescrPerson, useActualPositionName, showDetails) {

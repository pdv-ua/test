...
exports.reportCode = {
    ...
    return Promise.all([
      ...
      .then(([orderExtract]) => {
        return Promise.all([
          ...
          const whereArray = [['empOrderType', 'in', ['VACATION', 'VACATIONLONG']]]
          ...
          return Promise.all([
            ...
            UB.Repository('hr_empOrderActingDet')
              .attrs(['ID', 'paraID', 'dateFrom', 'dateTo', 'employeeID.fullFIO', 'employeeID.accusativeName',
            ...
            UB.Repository('hr_empOrderMaterialtransferDet')
              .attrs(['ID', 'employeePositionID', 'employeePositionID.employeeID.genName', 'employeePositionID.employeeID.fullFIO',
                'toEmployeePositionID.employeeID.datName', 'toEmployeePositionID.employeeID.fullFIO', 'toEmployeePositionID',
            ...
            UB.Repository('hr_commission')
              .attrs(['orderDetID', 'employeePositionID.employeeID.fullFIO', 'memberType.name', 'employeePositionID.employeeNumberID.tabNum'])
              .where('orderID', '=', reportParams.instanceID)
            ...
            UB.Repository('ac_settingsOrg')
              ...
              .where('[constantID.code]', 'in', ['hrOrderTabNum', 'hrOrderVacEmpGroup', 'hrEmpOrderMoveAbsentArticle', 'hrOrderVacExpWithoutPeriod'])
              .selectAsObject({
            ...
            UB.Repository('hr_empMilitaryRanks')
              ...
              .selectAsObject()
          ]).then(([configObj, empOrder, orderDet, orderLongDet, orderNext, orderAcc, orderResp, city, actDet, matTransfer, commission, settingsOrg, empMilitaryRanks]) => {
            ...
            return Promise.all([
              ...
              UB.Repository('hr_employeePositionS')
                .attrs(['ID', 'positionID.name', 'positionID.fullNameAcc', 'positionID.fullNameGen', 'positionID.nameAcc',
                  'positionID.mi_dateFrom', 'positionID.mi_dateTo', 'positionID.fullNameDat', 'positionID.nameDat', 'positionID.nameGen',
                ...
                .selectAsObject()
            ]).then(([task, employeeExperience, calendarHoliday, employeePosition]) => ({
              empOrder,
              orderDet,
              orderLongDet,
              orderNext,
  ...
  getParams: async function (data) {
    const me = this
    const orderPersons = []
    data.settingsOrg = data.settingsOrg ? _.groupBy(data.settingsOrg, 'code') : []
    const showTabNum = data.order.showTabNum // data.settingsOrg['hrOrderTabNum'] ? data.settingsOrg['hrOrderTabNum'][0].value === '1' : false
    const groupVacEmp = data.settingsOrg['hrOrderVacEmpGroup'] ? data.settingsOrg['hrOrderVacEmpGroup'][0].value === '1' : false
    const showArticle = data.settingsOrg['hrEmpOrderMoveAbsentArticle'] ? data.settingsOrg['hrEmpOrderMoveAbsentArticle'][0].value === '1' : false
    const experienceWithoutPeriod = data.settingsOrg['hrOrderVacExpWithoutPeriod'] ? data.settingsOrg['hrOrderVacExpWithoutPeriod'][0].value === '1' : false
    ...
    data.matTransfer = data.matTransfer && data.matTransfer.length ? _.groupBy(data.matTransfer, item => {
      return data.empOrder.find(el => el.employeePositionID === item.employeePositionID) ? item.employeePositionID : 0
    }) : []

    const params = {
      orderType: data.orderExtract && data.orderExtract.ID ? UB.i18n('В И Т Я Г &nbsp;З&nbsp; Н А К А З У') : UB.i18n('Н А К А З'),
      city: data.city,
    ...
    if (data.order.sortItems === 'STAFF') {
      const detailGroup = _.groupBy(orderPersons, item => item.depID)
      _.forEach(detailGroup, items => {
        const toOrder = data.orderExtract && data.orderExtract.ID ? (data.orderExtract.departmentID ? data.orderExtract.departmentID === items[0].depID : true) : true
        params.detail.push({
          toOrder: toOrder,
          ID: items[0].depID,
          idxNum: items[0].depIdxNum,
          name: items[0].depName,
          items: []
        })
      })
      params.detail = _.sortBy(params.detail, 'idxNum')
    } else {
      params.detail.push({
        toOrder: true,
        ID: -1,
    ...
    let index = 1
    _.forEach(params.detail, detail => {
      let fltOrderPersons = detail.ID === -1 ? orderPersons : orderPersons.filter(item => item.depID === detail.ID)
      if (groupVacEmp) {
        // orderPersons = _.sortBy(orderPersons, 'employeeID.fullFIO')
      ...
      _.forEach(fltOrderPersons, persons => {
        ...
        const position = HR.reportUtils.makePositionName(firstEl['employeePositionID.positionID.fullNameDat'] || firstEl['employeePositionID.positionID.nameDat'] || firstEl['employeePositionID.positionID.name'] || '', firstEl['employeePositionID.positionID.isOrgBoss'])

        /*
        let text = (_.size(orderPersons) === 1 && lenOrderPersons > 1 && (!data.matTransfer[0] || data.matTransfer[0].length === 0)) ||
        ((lenOrderPersons === 1 || lenOrderPersons === persons.length) && (!data.task || data.task.tasks.length === 0) && (!firstEl.act || firstEl.act.length === 0) &&
            (!data.matTransfer[firstEl.employeePositionID] || data.matTransfer[firstEl.employeePositionID].length === 0) &&
            (!data.matTransfer[0] || data.matTransfer[0].length === 0)) ? '' : `${index++}. `
        */
        ...
        let text = ((lenOrderPersons === 1 || lenOrderPersons === persons.length) && (!data.task || data.task.tasks.length === 0) &&
          (!data.matTransfer[0] || data.matTransfer[0].length === 0)) ? '' : `${index++}. `
        text += UB.i18n('НАДАТИ {0}<b>', militaryRank ? militaryRank + ' ' : '') + HR.reportUtils.formatFullNameInOrder(firstEl['employeeID.datName'] || firstEl['employeeID.fullFIO'], { notUseMiddleNameInOrder: params.notUseMiddleNameInOrder })
        text += (persons.length === 1 && showTabNum && firstEl['employeeNumberID.tabNum'] ? UB.i18n(` </b>(Таб. №&nbsp;{0})`, firstEl['employeeNumberID.tabNum']) : '</b>')
        text += position ? ', ' + position + orgGen : ''
        ...
        _.forEach(persons, el => {
          const personActingInfo = me.makeActing(el.act, (el['employeeID.genName'] || el['employeeID.fullFIO'] || ''), (showTabNum && el['employeeNumberID.tabNum'] ? UB.i18n(` (Таб. №&nbsp;{0})`, el['employeeNumberID.tabNum']) : ''),
            HR.reportUtils.makePositionName(el['employeePositionID.positionID.fullNameGen'] || el['employeePositionID.positionID.name'], el['employeePositionID.positionID.isOrgBoss']),
            data.employeePosition, el.dateFrom, el.dateTo, el['positionID.positionType'] === '1', showTabNum, /* persons.length */ lenOrderPersons > 1 ? 0 : index, orgGen, data.configObj.notUseMiddleNameInOrder)
          if (personActingInfo.length) {
            index += /* persons.length */ lenOrderPersons > 1 ? 0 : personActingInfo.length
        ...
        const matTransfer = []
        _.forEach(persons, el => {
          const personMatTransfer = data.matTransfer[el.employeePositionID] ? me.getMatTransferInfo(data.matTransfer[el.employeePositionID], data.commission, data.employeePosition, persons.length > 1 ? false : !(data.matTransfer[el.employeePositionID].length === 1 && lenOrderPersons === 0 && (data.task || data.task.tasks.length === 0)), showTabNum, index, orgGen, data.configObj.notUseMiddleNameInOrder) : []
          if (personMatTransfer.length) {
            index += /* persons.length */ lenOrderPersons > 1 ? 0 : personMatTransfer.length
    ...
    params.unrefMatTransfer = data.matTransfer[0] ? me.getMatTransferInfo(data.matTransfer[0], data.commission, data.employeePosition,
      (index !== 1 || (data.task && data.task.tasks.length !== 0)), showTabNum, index, orgGen, data.configObj.notUseMiddleNameInOrder) : []
    index += params.unrefMatTransfer.length

    params.detail = params.detail.filter(el => el.toOrder)

    params.tasks = data.task.tasks.map(e => ({
  ...
  getMatTransferInfo: function (matTransfer, commission, employeePosition, needIndex, showTabNum, index, orgGen, notUseMiddleNameInOrder) {
    ...
    matTransfer.forEach(matTransItem => {
      ...
      if (employeePosition[matTransItem.toEmployeePositionID]) {
        toPosName = HR.reportUtils.makePositionName(employeePosition[matTransItem.toEmployeePositionID][0]['positionID.fullNameDat'] ||
          employeePosition[matTransItem.toEmployeePositionID][0]['positionID.nameDat'] ||
          employeePosition[matTransItem.toEmployeePositionID][0]['positionID.name'] || '', employeePosition[matTransItem.toEmployeePositionID][0]['positionID.isOrgBoss'])
      }
      ...
      if (employeePosition[matTransItem.employeePositionID]) {
        fromPosName = HR.reportUtils.makePositionName(employeePosition[matTransItem.employeePositionID][0]['positionID.fullNameGen'] ||
          employeePosition[matTransItem.employeePositionID][0]['positionID.nameGen'] ||
          employeePosition[matTransItem.employeePositionID][0]['positionID.name'] || '', employeePosition[matTransItem.employeePositionID][0]['positionID.isOrgBoss'])
      }

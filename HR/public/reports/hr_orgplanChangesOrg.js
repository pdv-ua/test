...
exports.reportCode = {
  ...
  getReportData: async function (reportParams) {
    ...
    // const onDate = AC.dateService.shiftDate(reportParams.orderDate) || appAC.globalApplicationDate()
    const onDate = AC.dateService.shiftDate(reportParams.orderDate) || appAC.globalApplicationDate()
    ...
    const positionAttributes = ['ID', 'mi_data_id', 'parentUnitID', 'idxNum', 'dictPositionID', 'dictPositionID.name', 'dictStatePayID',
      ...
      'accrualSum', 'payElID.name', 'dictWagePayID.name', 'psCategory.name', 'dictStatePayID.name', 'reformer', 'dictPositionID.fullName',
    ...
    const staffTableData = await UB.Repository('hr_staffTable')
      .attrs(['orgID', 'orderDate', 'orderState', 'orgID.name', 'orgID.nameGen', 'entryOrderID.entryDate',
        'name', 'entryOrderID.orderDate', 'entryOrderID.orderNumber'])
    ...
    if (staffTableData['entryOrderID.orderDate'] && staffTableData['entryOrderID.orderNumber'] && staffTableData['entryOrderID.entryDate']) {
      result.titleOrder = UB.i18n(`(вводиться в дію з {0}`, AC.dateService.getStringFormatDate(staffTableData['entryOrderID.entryDate'], '', '')) +
    ...
    const positionOldOrgStruct = await UB.Repository('hr_position')
      .attrs(['quantity', 'accrualSum'])
      .where('orgID', '=', organizationID)
      // .where('mi_dateFrom', '<=', oldOnDate)
      // .where('mi_dateTo', '>=', oldOnDate)
      .where('state', '=', 'ACTIVE')
    ...
    let sums = me.countTotals(positionOldOrgStruct, result.roundTo, result.roundToQuantity, true)
    result.totalQuantity = sums.quantity
    result.totalFundpay = sums.fundpay
    result.oldTotalQuantity = sums.quantity
    result.oldTotalFundpay = sums.fundpay
    ...
    if (delUnits.length || addUnits.length || changedUnits.length) {
      ...
      sums = me.countTotals(pos, result.roundTo, result.roundToQuantity, true)
      result.totalQuantity -= sums.quantity
      result.totalFundpay -= sums.fundpay

      sums = me.countTotals(addPos, result.roundTo, result.roundToQuantity, true)
      result.totalQuantity += sums.quantity
      result.totalFundpay += sums.fundpay

      sums = me.countTotals(chgPos, result.roundTo, result.roundToQuantity, true)
      result.totalQuantity -= sums.quantity
      result.totalFundpay -= sums.fundpay

      const tree = me.generateDataForReport(organizationID, newOrgStruct, delUnits || [], changedUnits || [], addUnits || [], pos || [], chgPos || [], addPos || [], deptData, result.roundTo, result.roundToQuantity, entryOrderIDentryDate, showCases, oldOrgStruct)
      result.data = tree && tree.data ? tree.data : []
    }

    result.roundToQuantity = result.roundToQuantity || HR.reportUtils.getQuantityFractional(result.totalQuantity)
    result.roundToQuantityOld = result.roundToQuantity || HR.reportUtils.getQuantityFractional(result.oldTotalQuantity)
    return result
  },
  countTotals: function (items, roundTo, roundToQuantity, allItems) {
    const result = {
      quantity: 0,
      fundpay: 0
    }
    items.forEach(orgItem => {
      if (allItems || orgItem.mi_unityEntity === 'hr_position') {
        ...
        result.quantity += qnt
        result.fundpay += orgItem.accrualSum ? AC.currencyService.round(qnt * (orgItem.accrualSum || 0), roundTo === 'numberGroup' ? 0 : 2) : 0
      }
    })
    return result
  },
  ...
  generateDataForReport: function (itemID, orgStruct, delUnits, changedUnits, addUnits, delPos, changedPos, addPos, deptData, roundTo, roundToQuantity, entryOrderIDentryDate, showCases, oldOrgStructFull) {
    ...
    function makeUnit (currentUnit, unitDelete, unitAdd, unitChange, deptData, entryOrderIDentryDate, level) {
      ...
      if (unitChange && unitAdd) {
        ...
        attributes.forEach(item => {
          ...
          if (unitAdd[item.att] !== unitChange[item.att]) {
            if (item.att === 'parentUnitID') {
              const parentUnitItemA = _.find(orgStruct, { mi_data_id: unitAdd.parentUnitID })
        ...
        resultObj = {
          ...
          newValue: `${unitDelete ? 'припинення дії' : 'вступ в дію'}${entryOrderIDentryDate ? ' з ' + AC.dateService.formatDate(entryOrderIDentryDate) : ''}`,
    ...
    function addPosItems (posItem, chngItem, resultObj, type, entryOrderIDentryDate, roundTo, roundToQuantity) {
      const attributes = [
        ...
        { name: UB.i18n('Тип оплати'), att: 'paymentType.name', isBoolean: false },
      ...
      if (posItem) {
        ...
        const basepay = posItem.paymentType === 'CONTRACT' ? 0 : AC.currencyService.round(posItem.accrualSum || 0, roundTo === 'numberGroup' ? 0 : 2)
        if (chngItem) {
          attributes.forEach(item => {
            if (posItem[item.att] !== chngItem[item.att]) {
              if (item.att === 'parentUnitID') {
                const parentUnitItemA = _.find(orgStruct, { mi_data_id: posItem.parentUnitID })
                const parentUnitItemC = _.find(oldOrgStructFull, { mi_data_id: chngItem.parentUnitID })
                changesOld.push(`${item.name}: ${parentUnitItemC ? parentUnitItemC.fullName || '-' : '-'}`)
                changesNew.push(`${item.name}: ${parentUnitItemA ? parentUnitItemA.fullName || '-' : '-'}`)
              } else if (item.att === 'accrualSum') {
                changesOld.push(`${item.name}: ${chngItem.paymentType === 'CONTRACT' ? UB.i18n('Згідно умов трудового договору') : chngItem[item.att] || '-'}`)
                changesNew.push(`${item.name}: ${posItem.paymentType === 'CONTRACT' ? UB.i18n('Згідно умов трудового договору') : posItem[item.att] || '-'}`)
              } else {
          ...
          resultObj.data.push({
            ...
            quantity: qnt,
            basepay: basepay,
            fundpay: AC.currencyService.round(qnt * basepay, roundTo === 'numberGroup' ? 0 : 2),

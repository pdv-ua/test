  ...
  getReportData: async function (reportParams) {
    ...
    const requestData = await UB.Repository('hr_request')
      .attrs(['requestType', 'organizationID', 'requestNumber', 'mi_createDate', 'requestDate', 'organizationID.EDRPOUCode',
        ...
        'employeeNumberID.employeeID.middleName', 'employeeNumberID.employeeID.datName', 'employeeNumberID.employeeID.accusativeName',
        'employeeNumberID.employeeID.insName', 'employeeNumberID.employeeID.locName', 'employeeNumberID.employeeID.vocName',
        'employeeNumberID.employeeID.engName',
        ...
        'vacationKindID.nameGen', 'isMoneyHelp', 'dictRequestKindID.requestText', 'signatories',
        'dictRequestKindID.interval', 'dictRequestKindID.countRows' ])
    ...
    if (requestData.requestType === 'REQUEST_UNIVERSAL') {
      const Mustache = await SystemJS.import('mustache')
      const requestText = requestData['dictRequestKindID.requestText'] || ''
      const requestTextData = {
        reason: requestData.requestReason ? UB.i18n(`, у зв'язку із {0}`, HR.nameCase.uncap(requestData.requestReason)) : '',
        dateFrom: requestData.dateFrom ? AC.dateService.formatDate(requestData.dateFrom) : '',
        dateTo: requestData.dateTo ? AC.dateService.formatDate(requestData.dateTo) : '',
        firstName: requestData['employeeNumberID.employeeID.firstName'] || '',
        lastName: requestData['employeeNumberID.employeeID.lastName'] || '',
        middleName: requestData['employeeNumberID.employeeID.middleName'] || '',
        shortFIO: requestData['employeeNumberID.employeeID.shortFIO'] || '',
        fullFIO: requestData['employeeNumberID.employeeID.fullFIO'] || '',
        genName: requestData['employeeNumberID.employeeID.genName'] || '',
        datName: requestData['employeeNumberID.employeeID.datName'] || '',
        accusativeName: requestData['employeeNumberID.employeeID.accusativeName'] || '',
        insName: requestData['employeeNumberID.employeeID.insName'] || '',
        locName: requestData['employeeNumberID.employeeID.locName'] || '',
        vocName: requestData['employeeNumberID.employeeID.vocName'] || '',
        engName: requestData['employeeNumberID.employeeID.engName'] || '',
        dayCount: requestData['dayCount'] || ''
      }

      result.universal = {
        requestText: Mustache.render(requestText, requestTextData).replace(/&#39;/g, `'`).replace(/&/g, '&nbsp;').replace(/\n/g, '<br />'),
        reasonDocument: requestData.reasonDocument || null
      }
    }

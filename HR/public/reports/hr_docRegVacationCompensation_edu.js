...
exports.reportCode = {
  ...
  getParams: function (data, reportParams) {
    ...
    const result = {
      ...
      periodName: '',
      dayCount: data.docRegVacationCompensation['dayCount'] || '___',
      avgSum: data.docRegVacationCompensation['avgSum'] || 0,
      vacationPeriod: `${data.docRegVacationCompensation['dateFrom'] ? 'з ' + AC.dateService.formatDate(data.docRegVacationCompensation['dateFrom']) : ''}` +
        `${data.docRegVacationCompensation['dateTo'] ? ' по ' + AC.dateService.formatDate(data.docRegVacationCompensation['dateTo']) : ''}`,

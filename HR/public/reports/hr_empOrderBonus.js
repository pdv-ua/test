...
exports.reportCode = {
  ...
  getReportData: async function (ID, orderExtraID) {
    ...
    const result = {
      ...
      titleOrder: UB.i18n('Про нагородження'),
      items: []
    }
    ...
    const whereArray = [['empOrderType', '=', 'BONUS']]
    ...
    let detail = await UB.Repository('hr_empOrderBonusDet')
    ...
    if (detail.length === 1) {
      ...
      if (item.isExternal) {
        ...
        empName.posName = HR.reportUtils.makePositionName(empName.posName, item['employeePositionID.positionID.isOrgBoss'])
      }
      text += ` ${boldFormatBegin}${empName.empName}${boldFormatEnd}${empName.posName ? ', ' + empName.posName + orgGen : ''}.`
      if (toOrder) {
        result.items.push({
          text: text
        })
      ...
      result.titleOrder = item['bonusID.title'] ? item['bonusID.title'] : UB.i18n('Про нагородження ') + HR.nameCase.uncap(item['bonusID.name'])
    } else {
      detail = _.groupBy(detail, 'bonusID.name')
      _.forEach(detail, items => {
        if (_.size(detail) === 1) {
          result.titleOrder = items[0]['bonusID.title'] ? items[0]['bonusID.title'] : UB.i18n('Про нагородження ') + HR.nameCase.uncap(items[0]['bonusID.name'])
        }
        result.items.push({
          ...
          data: items.map((item, npp) => {
            ...
            return {
              toOrder: toOrder,
              text: `${boldFormatBegin}${empName.empName}${tabNum? ' ' + boldFormatEnd + tabNum : boldFormatEnd}${empName.posName ? ', ' + empName.posName + orgGen : ''}${npp === items.length - 1 ? '.' : ';'}`
            }

...
exports.reportCode = {
  ...
  getData (reportParams) {
    return Promise.all([
      UB.Repository('hr_docRegAvgMonth')
        .attrs(['ID', 'orderNumber', 'orderDate', 'countMonth', 'avgSumMonth', 'dateFrom',
          'employeePositionID', 'employeeID.fullFIO', 'employeeNumberID.tabNum', 'employeeNumberID', 'dateFromAvg', 'dateToAvg'])
      ...
      return Promise.all([
        UB.Repository('hr_employeePositionS')
          ...
          .where('positionID.mi_dateFrom', '<=', docRegAvgMonth.orderDate || docRegAvgMonth.dateFrom)
          .where('positionID.mi_dateTo', '>=', docRegAvgMonth.orderDate || docRegAvgMonth.dateFrom)
          .where('positionID.mi_deleteDate', '>=', '#maxdate')
          .joinCondition('departmentID.mi_dateFrom', '<=', docRegAvgMonth.orderDate || docRegAvgMonth.dateFrom)
          .joinCondition('departmentID.mi_dateTo', '>=', docRegAvgMonth.orderDate || docRegAvgMonth.dateFrom)
          .joinCondition('departmentID.mi_deleteDate', '>=', '#maxdate')
          .joinCondition('departmentID.state', '=', 'ACTIVE')
          .joinCondition('organizationID.mi_dateFrom', '<=', docRegAvgMonth.orderDate || docRegAvgMonth.dateFrom)
          .joinCondition('organizationID.mi_dateTo', '>=', docRegAvgMonth.orderDate || docRegAvgMonth.dateFrom)
          .joinCondition('organizationID.mi_deleteDate', '>=', '#maxdate')
          .joinCondition('organizationID.state', '=', 'ACTIVE')
          .selectById(docRegAvgMonth.employeePositionID),
        UB.Repository('hr_employeePositionS')
          .attrs(['ID', 'positionID.name', 'organizationID.name', 'departmentID.name'])
          .where('employeeID.mi_deleteDate', '>=', '#maxdate')
          .where('positionID.state', '=', 'ACTIVE')

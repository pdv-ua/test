...
exports.reportCode = {
  buildReport: function (reportParams) {
    ...
    if (UB.isServer) {
      let data = me.getDataOnServer(reportParams)
      let params = me.getParamsOnServer(data.mParams)
      let result = this.buildHTML(params)
      return this.transformToPdf(result)
    } else {
  ...
  getDataOnServer (reportParams) {
    ...
    global.hr_rl.getRL(ctx)
    return ctx
  },

  getParamsOnServer: function (data) {
    return global.hr_rl.getParamsOnServer(data)
  },
  ...
  getParams: function (data) {
    ...
    accrualArr.forEach(item => {
      if (item.days || item.hours || item.paySum) {
        ...
        const roundToRate = HR.reportUtils.getQuantityFractional(item.rate)
        accrual.push({
          payElNameAcc: item.description,
          paySumAcc: AC.currencyService.formatAsCurrency(item.paySum),
          periodAcc: `${AC.dateService.formatDate(dateFrom, 'dd.mm.yy')}-${AC.dateService.formatDate(dateTo, 'dd.mm.yy')}`,
          daysAcc: item.days || item.hours ? `${item.days ? item.days : 0}/${item.hours ? item.hours : 0}` : '',
          payRate: item.rate ? HR.reportUtils.quantityToString(item.rate, roundToRate) : ''
        })
      }
    })
    ...
    keepsArr.forEach(item => {
      if (item.paySum || item.rate) {
        ...
        keeps.push({
          ...
          periodKeep: `${AC.dateService.formatDate(dateFrom, 'dd.mm.yy')}-${AC.dateService.formatDate(dateTo, 'dd.mm.yy')}`,
          keepRate: item.rate ? HR.reportUtils.quantityToString(item.rate, roundToRate) : ''
        })
      }
    })
    ...
    params.payouts = payouts.map(item => {
      ...
      return {
        ...
        payoutPeriod: `${AC.dateService.formatDate(dateFrom, 'dd.mm.yy')}-${AC.dateService.formatDate(dateTo, 'dd.mm.yy')}`,

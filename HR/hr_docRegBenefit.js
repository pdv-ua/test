...
function afterInsert (ctx) {
  ...
  let dateFrom = dateService.shiftDate(execParams.orderDate)
  if (orderRegistry.dateFrom > dateFrom) {
    dateFrom = orderRegistry.dateFrom
  } else if (dateFrom > orderRegistry.dateTo) {
    dateFrom = orderRegistry.dateTo
  }
...
function beforeUpdate (ctx) {
  ...
  const orderRegistry = UB.Repository('hr_orderRegistry')
    .attrs(['periodID', 'periodID.dateFrom', 'periodID.dateTo', 'organizationID'])
    .selectById(orderRegistryDt.orderRegistryID || previousValues.orderRegistryID)
  orderRegistry.dateFrom = dateService.shiftDate(orderRegistry['periodID.dateFrom'])
  orderRegistry.dateTo = dateService.shiftDate(orderRegistry['periodID.dateTo'])
  let dateFrom = dateService.shiftDate(execParams.orderDate || previousValues.orderDate)
  if (orderRegistry.dateFrom > dateFrom) {
    dateFrom = orderRegistry.dateFrom
  } else if (dateFrom > orderRegistry.dateTo) {
    dateFrom = orderRegistry.dateTo
